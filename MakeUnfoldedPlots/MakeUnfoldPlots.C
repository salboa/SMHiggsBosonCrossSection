...
int main( int argc, char* argv[] ) {
  ...
  opt.VarFitFile = "fit_"+opt.Variable+"__wsysdata_TGunfolded.root";
  opt.SysFile    = "fit_"+opt.Variable+"__wsys_fixing_alldata_TGunfolded.root";
  m_Conf  = opt.isConf;
  m_Paper = opt.isPaper;
...
void MakeFidUnfoldPlots(std::string expFile, bool keepCanvas){
  ...
  TFile *_file1 = new TFile( "../ws_output/fit_m_fidchannel__wsysdata_TGunfolded.root" );
  TFile *_file2 = new TFile( "../ws_output/fit_m_fidsum__wsysdata_TGunfolded.root"     );
  TFile *_file3 = new TFile( "../ws_output/fit_m_fid4l__wsysdata_TGunfolded.root"      );
  TFile *_file4 = new TFile( "../ws_output/fit_m_fid2l2l__wsysdata_TGunfolded.root"    );
  TFile *_file5 = new TFile( "../ws_output/fit_m_total__wsysdata_TGunfolded.root"      );
  TFile *_file6 = new TFile( "../ws_output/fit_m_comb__wsysdata_TGunfolded.root"       );

  TFile *_file1sys = new TFile( "../ws_output/fit_m_fidchannel__wsys_fixing_alldata_TGunfolded.root" );
  TFile *_file2sys = new TFile( "../ws_output/fit_m_fidsum__wsys_fixing_alldata_TGunfolded.root"     );
  TFile *_file3sys = new TFile( "../ws_output/fit_m_fid4l__wsys_fixing_alldata_TGunfolded.root"      );
  TFile *_file4sys = new TFile( "../ws_output/fit_m_fid2l2l__wsys_fixing_alldata_TGunfolded.root"    );
  TFile *_file5sys = new TFile( "../ws_output/fit_m_total__wsys_fixing_alldata_TGunfolded.root"      );
  TFile *_file6sys = new TFile( "../ws_output/fit_m_comb__wsys_fixing_alldata_TGunfolded.root"       );
  ...
  leg->AddEntry(exp, "LHCXSWG ggH @N3LO + XH", "pf");
...
void MakeDiffUnfoldPlots(std::string var, std::string varFile,
  ...
  TH1F* dummy1 = CreateDummyPlot(mgexpected_ggH,"dummy1",var);
  dummy1->GetXaxis()->SetLabelOffset(1.8);
  TH1F* dummy2 = CreateDummyPlot(mgexpected_ggH,"dummy2",var);
  ...
  if (var=="pt"||var=="pt0j"||var=="pt1j"||var=="pt2j"||var=="ljpt"){
    ...
    dummy1->GetXaxis()->SetRangeUser(0,nnlops_rm->GetNbinsX());
  ...
  if (var=="m34"||var=="pt0j"){
    ...
    if(var=="m34"){
      obs->SetPoint(nnlops->GetNbinsX()-1, x, nnlops->GetBinContent(nnlops->GetNbinsX())*4.32);
    ...
    if(var=="pt0j"){
      obs->SetPoint(nnlops_rm->GetNbinsX()-1, x, nnlops_rm->GetBinContent(nnlops->GetNbinsX())*41.5);
  ...
  if (var!="njetv4"){
    ...
    if (var=="pt0j" || var=="pt1j" || var=="pt2j")
      ...
      pvaluennlops  = evalPValue(nnlops, "NNLOPS        ", var,normalized);

    double pvaluemg;
    if (var=="pt0j" || var=="pt1j" || var=="pt2j")
      pvaluemg = evalPValuePtPt("mg5",normalized);
    else
      pvaluemg = evalPValue(mg, "MadGraph      ", var,normalized);
    
    if(hresexpected_ggH!=NULL) pvaluehres = evalPValue(hres, "HRes          ", var,normalized);
    std::cout << "*******************************" << std::endl;
    ...
    if (var=="m34"||var=="pt0j"){
      ...
      if (var=="pt0j"){
        pVal.DrawLatex( x/dummy1->GetXaxis()->GetXmax()-0.04 , 0.08 ,"upper limit" );
