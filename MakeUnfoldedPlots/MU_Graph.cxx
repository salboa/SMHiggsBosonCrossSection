...
TGraphAsymmErrors* makeTGraph(TH1F* nominal,TH1F* up,TH1F* down,
  ...
  if(MCname=="NNLOPS"){
    ...
    else shift =  GetBarSpacing(m_Var);
  }
...
TGraphAsymmErrors* makeObs(TGraphAsymmErrors *obs, std::string var){
  ...
  for(int i=0; i < nbins; ++i){
    ...
    m_obs->SetPoint( i, x-offset, y /m_integralobs );
    m_obs->SetPointError( i, 0.0, 0.0,
			  obs->GetErrorYlow(i+offsetbin) / m_integralobs,
			  obs->GetErrorYhigh(i+offsetbin) / m_integralobs );
  }
...
TGraphAsymmErrors* makeObsSys(TGraphAsymmErrors *obs,
  ...
  for(int i=0; i < nbins; ++i){
    ...
    m_sys->SetPoint( i, x-offset, y / m_integralobs );  
    m_sys->SetPointError( i, GetBarSize(var).first, GetBarSize(var).first,//0.04, 0.04,
			  sqrt( pow(obs->GetErrorYlow(i+offsetbin),2) - pow(sys->GetErrorYlow(i+offsetbin),2) ) / m_integralobs,
			  sqrt( pow(obs->GetErrorYhigh(i+offsetbin),2) - pow(sys->GetErrorYhigh(i+offsetbin),2) ) / m_integralobs );
...
TGraphAsymmErrors* makeRatio(TGraphAsymmErrors *inGr,
  ...
  if(MC=="nnlops"  && (m_Var!="pt" && m_Var!="y") ) offset =  GetBarSpacing(var);
  ...
  for(Int_t ii=0; ii<inGr->GetN(); ii++){
    ...
    if(var=="mjj"){
      ...
      m_ratio->SetPoint(ii, dummy->GetBinCenter(ii+1)+offset,  y/y1 );
      ...
      if( y/y1 > 3 ||  y/y1 <0 )
	m_Arrows.push_back( MakeArrows(dummy->GetBinCenter(ii+1)+offset, y/y1, MC) );
    }
    else{
