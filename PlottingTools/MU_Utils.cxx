...
std::vector<float> totalXS(std::string mc){
  ...
  std::vector<std::string> prodmode = {"ggh", "vbf", "wh", "zh", "tth", "bbh","tWH","tHqb"} ;
  
  if(mc == "N3LO"){
    nominal   = {48.58, 3.782, 1.373, 0.8839, 0.5071, 0.488, 0.01517, 0.077129}; // ggF N3LO, vbf, wh, zh, tth, bbh, tWH, tHqb
    scaleup   = {  3.9,   0.4,   0.5,    3.8,    5.8,  20.2,     4.9,      6.9};
    scaledown = { -3.9,  -0.3,  -0.7,   -3.1,   -9.2, -23.9,    -6.7,    -15.0};
    pdfup     = {  3.2,   2.1,   1.9,    1.7,    3.6,     0,     6.1,      4.1};
    pdfdown   = { -3.2,  -2.1,  -1.9,   -1.7,   -3.6,     0,    -6.1,     -4.1};
  }
  else if (mc == "N2LO") {
    nominal   = { 42.3, 3.782, 1.373, 0.8839, 0.5071, 0.488, 0.01517, 0.077129}; // ggF NNLO+NNLL, vbf, wh, zh, tth, bbh, tWH, tHqb
    scaleup   = { 11.5,   0.4,   0.5,    3.8,    5.8,  20.2,     4.9,      6.9};
    scaledown = {-11.5,  -0.3,  -0.7,   -3.1,   -9.2, -23.9,    -6.7,    -15.0};
    pdfup     = {    0,   2.1,   1.9,    1.7,    3.6,     0,     6.1,      4.1};
    pdfdown   = {    0,  -2.1,  -1.9,   -1.7,   -3.6,     0,    -6.1,     -4.1};
  }
  else if (mc == "MG5") {
    nominal   = {32.75, 3.782, 1.373, 0.8839, 0.5071, 0.488, 0.01517, 0.077129}; // ggF NNLO+NNLL, vbf, wh, zh, tth, bbh, tWH, tHqb
    scaleup   = { 16.8,   0.4,   0.5,    3.8,    5.8,  20.2,     4.9,      6.9};
    scaledown = {-14.1,  -0.3,  -0.7,   -3.1,   -9.2, -23.9,    -6.7,    -15.0};
    pdfup     = {  1.7,   2.1,   1.9,    1.7,    3.6,     0,     6.1,      4.1};
    pdfdown   = { -1.7,  -2.1,  -1.9,   -1.7,   -3.6,     0,    -6.1,     -4.1};
  }
  else if (mc == "NNLOPS") {
    nominal   = {43.98, 3.782, 1.373, 0.8839, 0.5071, 0.488, 0.01517, 0.077129}; // ggF NNLO+NNLL, vbf, wh, zh, tth, bbh. tWH, tHqb
    scaleup   = {  9.5,   0.4,   0.5,    3.8,    5.8,  20.2,     4.9,      6.9};
    scaledown = { -9.5,  -0.3,  -0.7,   -3.1,   -9.2, -23.9,    -6.7,    -15.0};
    pdfup     = {  3.1,   2.1,   1.9,    1.7,    3.6,     0,     6.1,      4.1};
    pdfdown   = { -3.1,  -2.1,  -1.9,   -1.7,   -3.6,     0,    -6.1,     -4.1};
  }
...
double evalPValue(TGraphAsymmErrors* e, std::string MC, std::string var, bool normalized, TGraphAsymmErrors* g){
  ...
  TFile *_file0 = TFile::Open(("/home/andrea/MyProject/Analysis/Moriond2017/ws_output/"+var+"/ws_"+var+"_wsys_data.root").c_str());
  RooWorkspace *w = (RooWorkspace*) _file0->Get("myWS");
  RooStats::ModelConfig *mc = (RooStats::ModelConfig*) w->obj("ModelConfig");

  RooRealVar *tmp;
  TIterator *iterator= mc->GetGlobalObservables()->createIterator();
  while ( (tmp = (RooRealVar*)iterator->Next()) ) {
    if ( string(tmp->GetName()).find("ATLAS")==string::npos && (string(tmp->GetName()).find("BR")!=string::npos  || string(tmp->GetName()).find("A_")!=string::npos  /*|| string(tmp->GetName()).find("f_")!=string::npos*/) ){
      tmp->setVal(1);
      tmp->setConstant(1);
    }
  }
  ...
  double dummy, y;
  g->GetPoint(0,dummy,y);
  w->var("sigma_bin0_incl")->setVal(y*binwidth.at(0)); w->var("sigma_bin0_incl")->setConstant(1);
  g->GetPoint(1,dummy,y);
  w->var("sigma_bin1_incl")->setVal(y*binwidth.at(1)); w->var("sigma_bin1_incl")->setConstant(1);
  g->GetPoint(2,dummy,y);
  w->var("sigma_bin2_incl")->setVal(y*binwidth.at(2)); w->var("sigma_bin2_incl")->setConstant(1);
  g->GetPoint(3,dummy,y);
  w->var("sigma_bin3_incl")->setVal(y*binwidth.at(3)); w->var("sigma_bin3_incl")->setConstant(1);
  g->GetPoint(4,dummy,y);
  w->var("sigma_bin4_incl")->setVal(y*binwidth.at(4)); w->var("sigma_bin4_incl")->setConstant(1);


  if (var=="pt"){

    g->GetPoint(5,dummy,y);
    w->var("sigma_bin5_incl")->setVal(y*binwidth.at(5)); w->var("sigma_bin5_incl")->setConstant(1);
    g->GetPoint(6,dummy,y);
    w->var("sigma_bin6_incl")->setVal(y*binwidth.at(6)); w->var("sigma_bin6_incl")->setConstant(1);
    g->GetPoint(7,dummy,y);
    w->var("sigma_bin7_incl")->setVal(y*binwidth.at(7)); w->var("sigma_bin7_incl")->setConstant(1);
    g->GetPoint(8,dummy,y);
    w->var("sigma_bin8_incl")->setVal(y*binwidth.at(8)); w->var("sigma_bin8_incl")->setConstant(1);
    g->GetPoint(9,dummy,y);
    w->var("sigma_bin9_incl")->setVal(y*binwidth.at(9)); w->var("sigma_bin9_incl")->setConstant(1);
    g->GetPoint(10,dummy,y);
    w->var("sigma_bin10_incl")->setVal(y*binwidth.at(10)); w->var("sigma_bin10_incl")->setConstant(1);


  }
  int dof = binwidth.size();
  RooAbsData *p_data =  w->data("obsData");
  ...
  RooAbsReal *nll=simPdf->createNLL( *p_data, RooFit::CloneData(kFALSE), RooFit::Constrain(*mc->GetNuisanceParameters()),
                                        RooFit::Offset(kTRUE), RooFit::GlobalObservables(*mc->GetGlobalObservables()), RooFit::Verbose(kFALSE)/*, RooFit::NumCPU(1, RooFit::Hybrid) */);
  ...
  double unc = 2*nll->getVal();
      
  ////// set POI at the exp 
  //
  e->GetPoint(0,dummy,y);
  w->var("sigma_bin0_incl")->setVal(y*binwidth.at(0)); w->var("sigma_bin0_incl")->setConstant(1);
  e->GetPoint(1,dummy,y);
  w->var("sigma_bin1_incl")->setVal(y*binwidth.at(1)); w->var("sigma_bin1_incl")->setConstant(1);
  e->GetPoint(2,dummy,y);
  w->var("sigma_bin2_incl")->setVal(y*binwidth.at(2)); w->var("sigma_bin2_incl")->setConstant(1);
  e->GetPoint(3,dummy,y);
  w->var("sigma_bin3_incl")->setVal(y*binwidth.at(3)); w->var("sigma_bin3_incl")->setConstant(1);
  e->GetPoint(4,dummy,y);
  w->var("sigma_bin4_incl")->setVal(y*binwidth.at(4)); w->var("sigma_bin4_incl")->setConstant(1);


  if (var=="pt"){

    e->GetPoint(5,dummy,y);
    w->var("sigma_bin5_incl")->setVal(y*binwidth.at(5)); w->var("sigma_bin5_incl")->setConstant(1);
    e->GetPoint(6,dummy,y);
    w->var("sigma_bin6_incl")->setVal(y*binwidth.at(6)); w->var("sigma_bin6_incl")->setConstant(1);
    e->GetPoint(7,dummy,y);
    w->var("sigma_bin7_incl")->setVal(y*binwidth.at(7)); w->var("sigma_bin7_incl")->setConstant(1);
    e->GetPoint(8,dummy,y);
    w->var("sigma_bin8_incl")->setVal(y*binwidth.at(8)); w->var("sigma_bin8_incl")->setConstant(1);
    e->GetPoint(9,dummy,y);
    w->var("sigma_bin9_incl")->setVal(y*binwidth.at(9)); w->var("sigma_bin9_incl")->setConstant(1);
    e->GetPoint(10,dummy,y);
    w->var("sigma_bin10_incl")->setVal(y*binwidth.at(10)); w->var("sigma_bin10_incl")->setConstant(1);


  }
  RooSimultaneous *simPdf2 = (RooSimultaneous*) mc->GetPdf();
  RooAbsReal *nll2=simPdf2->createNLL( *p_data, RooFit::CloneData(kFALSE), RooFit::Constrain(*mc->GetNuisanceParameters()),
                                        RooFit::Offset(kTRUE), RooFit::GlobalObservables(*mc->GetGlobalObservables()), RooFit::Verbose(kFALSE)/*, RooFit::NumCPU(1, RooFit::Hybrid) */);
...
void evalPValueFid (std::vector<double> v){
  ...
  std::vector<std::string> scanname     = { "_4mu", "_4e", "_2mu2e", "_2e2mu",
					    "_4l", "_2l2l", "_sum", "", "" };
  std::vector<std::string> scanfilename = { "fidchannel_binbybin___sigma_4mu", "fidchannel_binbybin___sigma_4e",
					    "fidchannel_binbybin___sigma_2mu2e", "fidchannel_binbybin___sigma_2e2mu",
					    "fid4l_binbybin___sigma_4l", "fid2l2l_binbybin___sigma_2l2l",
					    "fidsum_binbybin___sigma_sum","comb_binbybin___sigma","total_binbybin___sigma"};
  
  std::vector<std::string> channels = {"4mu  ","4e   ", "2mu2e","2e2mu","4l   ","2l2l ","sum  ","comb ","total"};
  for(uint i=0; i<v.size(); ++i){
    
    //scanname = "sigma_bin"+std::to_string(i+offsetbin-1);
    TFile scanfile( ("../ws_output/m/""scan_m_"+scanfilename.at(i)+"_wsysdata.root").c_str()  );
